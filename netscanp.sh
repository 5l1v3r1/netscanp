#!/bin/bash
# Netscanp: Anonymous & Multi-threaded network port scan and printer spam in shellscript using Netcat e proxychains
# Author: thelinuxchoice
# Github: github.com/thelinuxchoice/netscanp
...
printf "\e[1;92m |_| \_|\___|\__|____/ \___\__,_|_| |_| \e[0m\e[1;77mPrinter Spam Ver.\e[0m\n"
printf "\n"
...
default_port=9100
read -p $'\e[1;37m[::] Port to scan\e[0m \e[1;91m(Default 9100):\e[0m ' port
...
read -p $'\e[1;37m[::] Threads \e[1;91m(Default 10):\e[0m \e[0m' threads2
threads2="${threads2:-${default_threads2}}"
startline=1
endline="$threads2"
count=0
count_target=$(wc -l $printer_file | cut -d " " -f1)
...
case "$1" in --resume) resume ;; --printer) printer ;; --scan) scan ;; *)
banner
